...
public class SQLHelper {
    ...
    public static int getCount(final String sql, final Connection connection,
		...
		countSql = selectSqlParseCountSql(sql, dbName);
...
//	        countSql = "select count(1) " + removeSelect(removeOrders(sql));
    ...
    /**
     * @Description: 将select查询语句转化成select count语句	
     ...
     * @date 2016年9月26日
     */
    public static String selectSqlParseCountSql(String sql, String dbName){
    	if("oracle".equals(dbName)){
    		sql = "select count(1) " + removeSelect(sql);
		}else{
			sql = "select count(1) " + removeSelect(removeOrders(sql));
		}
    	return sql;
    }
     ...
     * @return 分页SQL
     */
    public static String generatePageSql(String sql, Grid grid, Dialect dialect) {
        if (dialect.supportsLimit()) {
            return dialect.getLimitString(sql, grid.getOffset(), grid.getLimit());
        } else {
            return sql;
        }
    }
	...
	private static String removeSelect(String qlString){
		...
		if(count == 1){
			...
			int endPos = qlString.lastIndexOf("from ");
			if(beginPos != endPos){
				int wherePos = qlString.indexOf("where");
				if(endPos < wherePos){
					return qlString.substring(endPos);
				}else{
